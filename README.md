...
- [rodrigorobl/teachable-machine](https://github.com/rodrigorobl/teachable-machine) `📇` `🏠` – Real-time webcam image classifier powered by TensorFlow.js and Google Teachable Machine models, deployable on Vercel. **by [@rodrigorobl](https://github.com/rodrigorobl)**
...
- [andreferraz/gamemyvibe](https://github.com/andreferraz/gamemyvibe) `📇` `🏠` – A prototype game search/recommendation system based solely on game descriptions. It uses the Universal Sentence Encoder algorithm with Tensorflow, and run your "game vibe" comparison based on the descriptions. The web app fetches a pre-processed games list, calculates the embeddings for each game (directly in the browser), where then you can search by a given a type of scene or mood. **by [@andreferraz](https://github.com/andreferraz)**
